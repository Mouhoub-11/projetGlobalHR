			...
			for (int temp = 0; temp < employeeList.getLength(); temp++) {
				...
				if (employeeNode.getNodeType() == Node.ELEMENT_NODE) {
					...
					for (int i = 0; i < salaireList.getLength(); i++) {
					    Element salaire = (Element) salaireList.item(i);
					    if (salaire.getAttribute("idEmploye").equals(idEmploye)) {
					        String montantStr = salaire.getElementsByTagName("Montant").item(0).getTextContent();
					        double montant = Double.parseDouble(montantStr);
					        String dateVersement = salaire.getElementsByTagName("dateVersement").item(0).getTextContent();
					        System.out.println("salaire - Montant: " + montant + ", Date de versement: " + dateVersement);
					    }
					}
